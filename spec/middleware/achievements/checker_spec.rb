...
describe Achievements::Checker do
  ...
  describe '#new_rewards' do
    ...
    context 'when there is an achievement' do
      let(:condition) { create(:condition, action: activity.action, operation: :points, expression: :lt, value: 10_000) }
    ...
    context 'when there is more than one achievement' do
      let!(:condition) { create(:condition, action: activity.action, operation: :points, expression: :lt, value: 10_000) }
      let!(:condition2) { create(:condition, action: activity.action, operation: :counter, expression: :gte, value: 1) }
    ...
    context 'when an achievement has been already obtained' do
      let!(:condition) { create(:condition, action: activity.action, operation: :counter, expression: :gte, value: 1) }
